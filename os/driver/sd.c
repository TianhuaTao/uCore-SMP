...
#define SD_RESPONSE_BUSY 0x0
// Data token for commands 17, 18, 24
#define SD_DATA_TOKEN 0xfe
#define SD_START_BLOCK_TOKEN 0xfc
...
static inline uint8_t sd_txrx(spi_ctrl* spi, uint8_t data)
{
...
//        sd_read_blocks(spictrl, block_buf, i, 1);
...
int sd_write_blocks(spi_ctrl* spi, void* src, uint32_t dst_lba, size_t size)
{
    ...
    do {
        ...
        sd_txrx(spi, SD_START_BLOCK_TOKEN);
        do {
            uint8_t x = *p++;
            sd_txrx(spi, x);
            crc = crc16(crc, x);
    ...
    sd_txrx(spi, SD_STOP_TRAN_TOKEN);
